...
def get_naver_news_articles():
    ...
    if not articles:
        articles = [
            {"title": "정부지원금 및 생활 복지 주요 제도 변경 및 혜택 안내", "description": "올해 새롭게 시행되는 기준과 필수 신청 자격 조건 총정리", "date": "2026"}
        ]
    return articles
...
def get_golden_keyword_and_source():
    ...
    prompt = f"""
    다음은 [정부지원금 및 생활 복지] 분야의 최신 뉴스 및 정책 헤드라인 목록입니다:
    ...
    try:
        ...
        data = json.loads(res)
        kw = data.get("golden_keyword", "정부지원금 및 생활 복지 핵심 가이드")
    except Exception as e:
        kw = "정부지원금 및 생활 복지 필수 정보"
